...
# Calculate total elapsed seconds and per-ticket latency safely
elapsed_sec = 0.0
sec_per_ticket = 0.0
if len(timestamps) >= 2:
    timestamps.sort()
...
pending_queue = len(
    [
        ...
        if (item.get("ticket") or {}).get("ticket_id")
        not in st.session_state.decisions
    ]
)
        ...
        for idx, item in enumerate(pending_items):
            ...
            with col_cust:
                ...
                st.markdown(
                    f"""
                    <div class="flag-box">
                        <b style="color:#B76E00;">⚠️ FLAGGED BECAUSE:</b><br>
                        {reason}
                    </div>
                    """,
                    unsafe_allow_html=True,
...
# ==================================================
# TAB 2: BULLETPROOF AUDITi TRAIL & TELEMETRY TRACE
# ==================================================
with tab_audit:
    ...
    if not events:
        ...
        # Group events by ticket_id safely
        tickets_map = {}
        for ev in events:
            t_id = ev.get("ticket_id") or "System_Level"
        ...
        if selected_ticket:
            ...
            for ev in ticket_events:
                ...
                if event_type == "triage_complete":
                    with st.expander(f"🧭 [{timestamp}] Step 1: Triage Classification Complete", expanded=True):
                ...
                # Step 4: Critic QA
                    ...
                    with st.expander(f"🔍 [{timestamp}] Step 4: Critic QA Audit", expanded=True):
